...
Impedance Control Law:
    F = K * (x_des - x) - D * xdot           (Cartesian force)
    tau = J^T * F + g(q) - D_joint * qdot    (with joint damping)
...
# -------------------------------------------------
x_des = np.array([-0.15, 0.10])  # 2D target (X, Y) in meters
...
D_val = 10.0     # Damping (Ns/m) - overdamped for stability

K = np.diag([K_val, K_val])    # Stiffness matrix (N/m)
D = np.diag([D_val, D_val])    # Damping matrix (Ns/m)

# -------------------------------------------------
# Joint-Space Damping (to reduce joint oscillations)
# -------------------------------------------------
# Since D_effective = J^T * D * J is non-uniform across joints,
# we add explicit joint damping to stabilize each joint independently.
# Increase these values for joints that oscillate more.
#
#                       Joint1   Joint2   Joint3 (gripper)
D_joint = np.array([    0.1,     0.1,     0.1   ])  # Nm*s/rad
...
print("-" * 65)
print(f"Cartesian Stiffness K: {K[0,0]:.1f} N/m")
print(f"Cartesian Damping D:   {D[0,0]:.1f} Ns/m")
print(f"Joint Damping D_joint: [{D_joint[0]:.2f}, {D_joint[1]:.2f}, {D_joint[2]:.2f}] Nm*s/rad")
print("-" * 65)
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and sim_time < max_sim_time:
        ...
        # --- Joint-space damping (reduces oscillations per joint) ---
        # This directly damps each joint's velocity independently
        # tau_joint_damping = -D_joint * qdot
        tau_joint_damping = -D_joint * qdot  # Shape: (3,)

        # --- Total torque ---
        # tau = J^T*F + gravity + joint_damping
