"""
Cartesian Impedance Control - Custom Jacobian Version 2
Uses Robot class from robot_model.py for FK, Jacobian, IK
WITH joint damping

Control Law:
    F = K * (x_des - x) - D * xdot
    tau = J^T * F + g(q) - D_joint * qdot
"""
...
D_joint = np.array([0.1, 0.1, 0.1])  # Joint damping (Nm*s/rad)
...
print("=" * 65)
print("CUSTOM JACOBIAN V2 (Using robot_model.py - With Joint Damping)")
print("=" * 65)
...
print(f"K={K[0,0]} N/m, D={D[0,0]} Ns/m, D_joint={D_joint}")
print("=" * 65)
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and sim_time < max_sim_time:
        ...
        # Gravity compensation + joint damping
